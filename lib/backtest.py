...
def tradeBracket(price, entryBar, upper=None, lower=None, timeout=None):
    ...
    if upper:
        ...
        idx = np.where(p > upper)[
            0
        ]  # find where price is higher than the upper bracket
        if idx.any():
...
class Backtest(object):
    ...
    def __init__(
        self, price, signal, signalType="capital", initialCash=0, roundShares=True
    ):
        """
        ...
        assert signalType in [
            "capital",
            "shares",
        ], "Wrong signal type provided, must be 'capital' or 'shares'"
        ...
        if signalType == "shares":
            self.trades = self.signal[
                tradeIdx
            ]  # selected rows where tradeDir changes value. trades are in Shares
        ...
        self.data = pd.DataFrame(
            index=price.index, columns=["price", "shares", "value", "cash", "pnl"]
        )
        ...
        self.data["cash"] = (-delta * self.data["price"]).fillna(
            0
        ).cumsum() + initialCash
    ...
    def sharpe(self):
        ...
        pnl = (self.data["pnl"].diff()).shift(-1)[
            self.data["shares"] != 0
        ]  # use only days with position.
