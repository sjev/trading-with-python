...
import os
from . import yahooFinance as yf
...
def downloadUrl(date):
    ...
    url = s.substitute(
        fName=date2fileName(date), year=date.year, month="%02d" % date.month
    )
    return url
...
class BATS_Data(object):
    ...
    def _missingDates(self):
        ...
        print("Getting yahoo data to determine business dates... ", end=" ")
    ...
    def loadDate(self, date):
        ...
        buf = io.StringIO(lines)  # create buffer
        df = pd.read_csv(
            buf,
            sep="|",
            index_col=1,
            ...
            dtype={
                "Date": object,
                "Short Volume": np.float32,
                "Total Volume": np.float32,
            },
        )
