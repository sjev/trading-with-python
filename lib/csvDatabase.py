...
class HistDataCsv(object):
    ...
    def saveData(self, df, lowerCaseColumns=True):
        ...
        if (
            lowerCaseColumns
        ):  # this should provide consistency to column names. All lowercase
            df.columns = [c.lower() for c in df.columns]

        s = (
            self.symbol + "_" + df.index[-1].strftime(dateTimeFormat) + ".csv"
        )  # file name
...
class HistDatabase(object):
    ...
    def loadDates(self, dates=None):
        ...
        """
        if dates is None:
            dates = self.commonDates

        tmp = {}

        for k, v in list(self.csv.items()):
    ...
    def commonDates(self):
        ...
        t = [v.dates for v in list(self.csv.values())]  # get all dates in a list
...
if __name__ == "__main__":
    ...
    #
    pair = pd.DataFrame(
        {"SPY": spy.loadDate(date)["close"], "VXX": vxx.loadDate(date)["close"]}
    )
