...
class Plot:
    ...
    def candlestick(self, df):
        ...
        w = (
            np.median(np.diff(df.index)) / np.timedelta64(1, "ms") / 1.5
        )  # bar width in ms
    ...
    def triangle(self, series, orientation="up", **kwargs):
        ...
        angles = {"up": 0, "down": 45}
        self.fig.triangle(
            series.index, series.values, angle=angles[orientation], size=10, **kwargs
        )
...
def linkedPlots(
    ...
    for idx, (x, y) in enumerate(xy):
        ...
        fig.circle(x, y, source=source, selection_color="firebrick", fill_color="white")
        figures.append(fig)
