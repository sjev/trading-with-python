...
class DataFrameModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        col = self.df.ix[
            :, index.column()
        ]  # get a column slice first to get the right data type
        elm = col[index.row()]
        # elm = self.df.ix[index.row(),index.column()]

        if self.df.columns[index.column()] in list(self.columnFormat.keys()):
