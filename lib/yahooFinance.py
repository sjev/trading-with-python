...
def getHistoricData(symbols, delay=0.5, **options):
    ...
    assert isinstance(
        symbols, (list, str)
    ), "Input must be a string symbol or a list of symbols"
...
def getSymbolData(
    symbol, sDate=(2000, 1, 1), adjust=False, verbose=True, dumpDest=None
):
    """
    get data from Yahoo finance and return pandas dataframe
    ...
    df = pd.read_csv(
        buf, index_col=0, parse_dates=True, na_values=["null"]
    )  # convert to pandas DataFrame

    # rename columns
    newNames = [c.lower().replace(" ", "_") for c in df.columns]
    renames = dict(list(zip(df.columns, newNames)))
...
def loadToken():
    ...
    try:  # load file from disk
        ...
        age = (
            dt.datetime.now() - dt.datetime.strptime(data["timestamp"], dateTimeFormat)
        ).days
