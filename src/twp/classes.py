...
class Symbol(object):
    ...
    def downloadHistData(
        self,
        startDate=(2010, 1, 1),
        endDate=date.today().timetuple()[:3],
        source="yahoo",
    ):
        """
    ...
    def __init__(self, stock, hedge, beta=None):
        ...
        if isinstance(stock, str) and isinstance(hedge, str):
            ...
            self.price = pd.DataFrame(
                dict(list(zip(self.symbols, [stock, hedge])))
            ).dropna()
        else:
            raise ValueError(
                "Both stock and hedge should be of the same type, symbol string or Series"
            )
        ...
        self.data["beta"] = pd.Series(
            {self.symbols[0]: 1.0, self.symbols[1]: -self.beta}
        )
