...
def testDownload():
    ...
    data = DL.requestData(
        contract, end, durationStr="6 M", barSizeSetting="1 day", whatToShow="TRADES"
    )
...
def getParser():
    ...
    parser.add_argument(
        "--symbols", help="symbols separated by comma: SPY,VXX", default="all"
    )
    parser.add_argument(
        "--end",
        help="timestamp from where to start download.\
                         Defaults to last trading date",
        default=None,
    )
...
if __name__ == "__main__":
    ...
    SETTINGS["getSymbols"] = (
        list(SETTINGS["subscriptions"].keys())
        if args["symbols"] == "all"
